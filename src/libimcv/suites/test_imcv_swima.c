...
static sw_inv_data_t sw_inv_data[] = {
	...
	{ IETF_SWIMA_ATTR_SW_INV_FLAG_NONE, 0xaabbccd2, 0x12345678, 0x00000030,
	  chunk_from_chars(
		...
		0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x1F, 0x73, 0x74,
...
START_TEST(test_imcv_swima_inv)
{
	...
	attr = ietf_swima_attr_sw_inv_create(sw_inv_data[_i].flags,
										 ...
										 sw_inv_data[_i].eid_epoch,
										 sw_inv_data[_i].last_eid, sw_id_only);
	...
	ck_assert(c_attr->get_last_eid(c_attr,   NULL) == sw_inv_data[_i].last_eid);
	ck_assert(c_attr->get_last_eid(c_attr, &epoch) == sw_inv_data[_i].last_eid);
...
START_TEST(test_imcv_swima_sw_collector)
{
	...
	inventory = collector->collect(collector, TRUE, targets);
	...
	while (enumerator->enumerate(enumerator, &sw_record))
	{
		...
		if ( item == 1 || item == items)
		{
			...
			{
				DBG1(DBG_IMC, "          "
							  "locator: %.*s", sw_locator.len, sw_locator.ptr);
			}
	...
	inventory = collector->collect(collector, FALSE, targets);
	items = inventory->get_count(inventory);
